  ...
  public void createControl(Composite parent) {
    ...
    buttonAdd.addSelectionListener(new SelectionAdapter() {
      ...
      private Map<String, List<String>> verifyAndFilterGroups(Map<String, List<String>> groups) {
        String lightRegex = ConfigurationScope.INSTANCE.getNode(PrefNodes.REGEX_PATTERNS).get(
            RegexPatterns.LABVIEW_LIGHT, "");
      ...
      private Map<String, List<String>> groupNames(String[] names) {
        String regex = ConfigurationScope.INSTANCE.getNode(PrefNodes.REGEX_PATTERNS).get(
            RegexPatterns.LABVIEW_ENDING, "");

        return Arrays.stream(names).collect(
            Collectors.groupingBy(item -> item.replaceFirst(regex, "")));
      }

      private String[] filterNames(String[] names) {
        final String fileRegex = ConfigurationScope.INSTANCE.getNode(PrefNodes.REGEX_PATTERNS).get(
            RegexPatterns.LABVIEW_FILE, "");
      ...
      private void checkNamesForLabviewFiles(String[] names) {
        ...
        List<String> fails = Arrays.stream(names).filter(name -> !name.matches(fileRegex)).collect(
            Collectors.toList());
      ...
      public void widgetSelected(SelectionEvent e) {
        ...
        if (selection.isEmpty() == false) {
          ...
          DoubleInputDialog dialog = new DoubleInputDialog(buttonsGroup.getShell(), "Set Area...",
              "Input area [cm^2] to set...", 0D);

          if (dialog.open() == Window.OK) {
            selection.iterator().forEachRemaining(
                data -> ((LabviewDataFile) data).setArea(dialog.getValue() / 10000));
            viewer.refresh();
          }
    ...
    setPowerInputButton.addSelectionListener(new SelectionAdapter() {
      ...
      public void widgetSelected(SelectionEvent e) {
        ...
        if (selection.isEmpty() == false) {
          ...
          DoubleInputDialog dialog = new DoubleInputDialog(buttonsGroup.getShell(),
              "Set PowerInput...", "PowerInput to set...", 0D);

          if (dialog.open() == Window.OK) {
            selection.iterator().forEachRemaining(
                data -> ((LabviewDataFile) data).setPowerInput(dialog.getValue()));
            viewer.refresh();
          }
