...
public class ExportToExcelHandler {
  ...
  public void execute(@Named("TreeViewer") TreeViewer viewer, Shell shell) {
    ...
    fd.setFilterNames(new String[] {"Ms Excel Open XML Format Spreadsheet"});
    ...
    if (fileName != null) {
      ...
      ExcelOutputHelper helper =
          new ExcelOutputHelper(new ArrayList<CellResult>(cellResults), fileName);
      helper.execute();
    }
  }
  ...
  public boolean canExecute(@Named("TreeViewer") TreeViewer viewer) {
    ...
    return selection.toList().stream().anyMatch(
        arg -> (arg instanceof CellResult) || (arg instanceof CellGroup));
